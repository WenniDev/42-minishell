...
int	check_exec(t_data *data, int i)
{
	int	stat;

	if (data->c_table[i].exec_if)
		waitpid(data->c_table[i - 1].pid, &stat, 0);
...
int	exec(t_data *data)
{
	...
	{
		io_red(data, i);
		if (data->c_nb == 1 && data->c_table[i].type == BUILTIN)
			return (exec_builtin(data));
		if (check_exec(data, i))
			data->c_table[i].pid = fork();
		if (data->c_table[i].pid == ERROR)
		{
			sperr(data, NULL, "fork", errno);
			exit_prg(data);
		}
		if (!data->c_table[i].pid)
			exec_cmd(data, data->c_table[i]);
		i++;
	}
	reset_io(data);
	return (wait_cmds(data));
}
